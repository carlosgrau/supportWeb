...
moduleFactura.controller('facturaPlistController', ['$scope', 'toolService', '$http', 'sessionService', '$routeParams', '$location',
    function ($scope, toolService, $http, sessionService, $routeParams, $location) {
        ...
        $http({
            method: 'GET',
            url: host + 'json?ob=factura&op=getcount&ejercicio=' + sessionService.getEmpresa()
        ...
        $http({
            method: 'GET',
            url: host + 'json?ob=factura&op=getpage&ejercicio=' + sessionService.getEmpresa() + '&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        }).then(function (response) {
            $scope.status = response.status;
